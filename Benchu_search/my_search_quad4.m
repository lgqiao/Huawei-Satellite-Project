function [Path,Min_len] = my_search_quad4(map,W,t)
%MY_SEARCH_QUAD4: Calculate the shortest path from a source node to other nodes in an area
%                 the fourth quadrant direction
...
if xSource==xDestination || ySource==yDestination
    ...
    if isempty(point_sy_end)==0 
        % Area 1
        if isempty(path_valley1)==0
            y_range = point_hor(1,2):point_hor1(1,2)-1;
            boundary_ver2(:,2) = point_hor(end,2)+1:point_hor1(end,2);
        else
            point_hor1 = [];
            y_range = point_hor(1,2):yDestination;
            boundary_ver2(:,2) = point_hor(end,2)+1:yDestination;
        end
        ...
        % Area 2
        if isempty(path_valley2)==0
            x_range = point_ver(1,1):point_ver2(1,1)-1;
            boundary_hor2(:,1) = point_ver(end,1)+1:point_ver2(end,1);
        else
            point_ver2 = [];
            x_range = point_ver(1,1):xDestination;
            boundary_hor2(:,1) = point_ver(end,1)+1:xDestination;
        end
        ...
        if isempty(path_valley1)==0
            area3_1 = point_ver1;
            area3_2(:,1) = xSource:point_ver1(end,1);
        ...
        if isempty(path_valley2)==0
            area4_1 = point_hor2;
            area4_2(:,2) = ySource:point_hor2(end,2);
        ...
        elseif isempty(path_valley1)==0
            % Area 1
            area1_1 = point_ver1;
            area1_2(:,1) = xSource:point_ver1(end,1);
            area1_2(:,2) = yDestination;
            [Path_area1,Len_area1] = path_generate_ver(area1_1,area1_2,W);

            % Area 2
            y_range1 = ySource:point_hor1(1,2)-1;
            boundary_ver1(:,2) = y_range1;
            boundary_ver1(:,1) = xSource;
            area2_1 = [boundary_ver1;point_hor1];

            y_range2 = ySource:point_hor1(end,2);
            area2_2(:,2) = y_range2;
            area2_2(:,1) = xDestination;
            [Path_area2,Len_area2] = path_generate_hor(area2_1,area2_2,W);
        elseif isempty(path_valley2)==0
            % Area 1
            x_range1 = xSource:point_ver2(1,1)-1;
            boundary_hor1(:,1) = x_range1;
            boundary_hor1(:,2) = ySource;
            area1_1 = [boundary_hor1;point_ver2];

            x_range2 = xSource:point_ver2(end,1);
            area1_2(:,1) = x_range2;
            area1_2(:,2) = yDestination;
            [Path_area1,Len_area1] = path_generate_ver(area1_1,area1_2,W);
            ...
            % Area 2
            area2_1 = point_hor2;
            area2_2(:,2) = ySource:point_hor2(end,2);
            area2_2(:,1) = xDestination;
            [Path_area2,Len_area2] = path_generate_hor(area2_1,area2_2,W);
        end
    end
    ...
    if isempty(point_sy_end)==0 % The rectangle has a symmetrical dividing line
        ...
        y_range1 = ySource:point_hor(1,2)-1;
        ...
        for i = 1:sz_path_ver
            len_row = area1_2(i,1)-area1_1(i,1)+1;
        ...
        x_range1 = xSource:point_ver(1,1)-1;
        ...
        for i = 1:sz_path_hor
            len_col = area2_2(i,2)-area2_1(i,2)+1;
        ...
        if isempty(path_valley1)==0
            ...
            for i = 1:size(point_ver1,1)
                len_col = area3_2(i,2)-area3_1(i,2)+1;
        ...
        if isempty(path_valley2)==0
            ...
            for i = 1:size(point_hor2,1)
                len_row = area4_2(i,1)-area4_1(i,1)+1;
        ...
        if isempty(path_valley1)==1 && isempty(path_valley2)==1 % No valley lines go through the rectangle
            if W(xDestination+1,ySource+1)<W(xSource+1,yDestination+1) % case 1(Area 1)
                ...
                sz_path_hor = size(path_hor,1);
                for i = 1:sz_path_hor
                    len_col = area1_2(i,2)-area1_1(i,2)+1;
                    path_hor_val = path_hor(1:i,:);
                    len_hor_val = sum(len_hor(1:i-1));
                    for j = 1:len_col
                        k = k+1;
                ...
                sz_path_ver = size(path_ver,1);
                for i = 1:sz_path_ver
                    len_row = area2_2(i,1)-area2_1(i,1)+1;
                    path_ver_val = path_ver(1:i,:);
                    len_ver_val = sum(len_col(1:i-1));
                    for j = 1:len_row
                        k = k+1;

function [Path,Min_len] = my_search_quad1(map,W,t)
%MY_SEARCH_QUAD1: Calculate the shortest path from a source node to other nodes in an area,
...
if xSource==xDestination || ySource==yDestination
    ...
    MAP(xSource+1:xDestination+1,yDestination+1:ySource+1) = 2;
...
%     % If the duplicate path node on the right border of the rectangle contains the vertex
%     % in the upper right corner of the rectangle, remove this point to prevent double calculation
%     if sz_node_ver>0
...
%     end
    ...
    for i = 1:sz_node_hor
        k_hor = find(x_hor>=duplicate_hor(i,2) & x_hor<=duplicate_hor(i,1));
        x_hor(k_hor) = [];
    end
    for i = 1:sz_node_ver
        k_ver = find(y_ver>=duplicate_ver(i,1) & y_ver<=duplicate_ver(i,2));
        y_ver(k_ver) = [];
    end
    ...
    % on the left and lower bounds of the rectangle
    % left bound
    ...
    MAP(xSource+1,yDestination+1:ySource) = 1;
    ...
    MAP(xSource+2:xDestination+1,ySource+1) = 1;

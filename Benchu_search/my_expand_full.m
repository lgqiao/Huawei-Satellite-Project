function [Path,Path_expand,Min_len] = my_expand_full(vertex,Source,W,t)
%MY_EXPAND_FULL: Compute shortest paths from one source node to other nodes in an area in the extended map
...
if x_s>x1 && x_s<x2 && y_s>y1 && y_s<y2 % case1, the source node is inside the rectangle
    ...
    [Path1,Path_expand1,Min_len1,index_hor1,index_ver1] = my_expand_quad(map1,W,t);
    ...
    [Path2,Path_expand2,Min_len2,~,~] = my_expand_quad(map2,W,t);

    % third quadrant
    map3 = map_construct(W,Source,vertex3);
    [Path3,Path_expand3,Min_len3,index_hor3,index_ver3] = my_expand_quad(map3,W,t);
    ...
    [Path4,Path_expand4,Min_len4,~,~] = my_expand_quad(map4,W,t);
...
elseif x_s>x1 && x_s<x2 && y_s==y1 % case2, the source node is on the upper bound of the rectangle
    % third quadrant
    map3 = map_construct(W,Source,vertex3);
    [Path3,Path_expand3,Min_len3,~,index_ver3] = my_expand_quad(map3,W,t);
...
elseif x_s>x1 && x_s<x2 && y_s==y2 % case3, the source node is on the lower bound of the rectangle
    % first quadrant
    map1 = map_construct(W,Source,vertex1);
    [Path1,Path_expand1,Min_len1,~,index_ver1] = my_expand_quad(map1,W,t);
...
elseif y_s>y1 && y_s<y2 && x_s==x1 % case4, the source node is on the left bound of the rectangle
    % first quadrant
    map1 = map_construct(W,Source,vertex1);
    [Path1,Path_expand1,Min_len1,index_hor1,~] = my_expand_quad(map1,W,t);
...
elseif y_s>y1 && y_s<y2 && x_s==x2 % case5, the source node is on the right bound of the rectangle
    ...
    % third quadrant
    map3 = map_construct(W,Source,vertex3);
    [Path3,Path_expand3,Min_len3,index_hor3,~] = my_expand_quad(map3,W,t);

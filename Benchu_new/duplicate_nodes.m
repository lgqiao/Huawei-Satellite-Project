function [duplicate_hor,duplicate_ver] = duplicate_nodes(x1,y1,x2,y2,t)
%DUPLICATE_NODES: Find the endpoints of the duplicate path at the upper(lower) and right(left) 
...
if dx>0 % forward
    ...
    for i = 1:length(x_hor)
        [~,~,delta_x,~] = inter_sd(x1,y1,x_hor(i),y2,t);
    ...
    for i = 1:length(y_ver)
        [~,~,~,delta_y] = inter_sd(x1,y1,x2,y_ver(i),t);

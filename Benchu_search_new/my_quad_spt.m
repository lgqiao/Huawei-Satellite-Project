function [Path,Path_expand,Min_len,index_hor,index_ver] = my_quad_spt(Source,vertex,W,t)
%MY_QUAD_SPT: Compute the shortest path from the source node to other 
...
xDestination = vertex(1);
yDestination = vertex(2);
...
if xSource==xDestination
    ...
    [path_expand1,len1] = path_generate_ex(x_s,y_s,x_d1,y_d1,W_expand,1);
    [path_expand2,len2] = path_generate_ex(x_s,y_s,x_d2,y_d2,W_expand,1);
...
elseif ySource==yDestination
    ...
    [area_inner,area_outer] = area_vertex(xSource,ySource,xDestination,yDestination,max_x,max_y);
    ...
    if area_outer(1,1)>=area_inner(1,1) && area_outer(1,2)>=area_inner(1,2)
        boundary1 = area_boundary(x_s,y_s,area_outer(1,1),area_outer(1,2),W,t);
    ...
    if area_outer(2,1)<=area_inner(2,1) && area_outer(2,2)<=area_inner(2,2)
        boundary2 = area_boundary(x_s,y_s,area_outer(2,1),area_outer(2,2),W,t);
    ...
    if area_outer(3,1)>=area_inner(3,1) && area_outer(3,2)<=area_inner(3,2)
        boundary3 = area_boundary(x_s,y_s,area_outer(3,1),area_outer(3,2),W,t);
    ...
    if area_outer(4,1)<=area_inner(4,1) && area_outer(4,2)>=area_inner(4,2)
        boundary4 = area_boundary(x_s,y_s,area_outer(4,1),area_outer(4,2),W,t);
